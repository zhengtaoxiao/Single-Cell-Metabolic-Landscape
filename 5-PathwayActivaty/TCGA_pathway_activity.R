...
g <- ggplot(bulk_df,aes(x=Var2,y=value,fill=Var2)) +
  geom_violin(trim=F,size=0.2,show.legend = F,width=0.8) + labs(y=NULL,x=NULL) + 
...
ggsave(file.path(outDir,"pathway_activity_violinplot.pdf"),g,width = 0.8,height=1.5,units="in",device="pdf",useDingbats=FALSE)
...
dat <- cbind(sc_dat[sort_row,"Malignant",drop=T], dat_order[,"tumor",drop=T])
