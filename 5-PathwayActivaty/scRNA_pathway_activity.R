...
for(p in pathway_names){
  ...
  pathway_number_weight = 1 / gene_pathway_number[genes_comm,]
  ...
  ##shuffle 5000 times:  
  tmp = foreach(i = 1:5000) %dopar% {
    shuffle_cell_types <- sample(all_cell_types) ##shuffle the labels of the cell types
    mean_exp_eachCellType <- apply(pathway_metabolic_tpm, 1, function(x)by(x, shuffle_cell_types, mean))
    ratio_exp_eachCellType <- t(mean_exp_eachCellType) / colMeans(mean_exp_eachCellType)
    mean_exp_pathway <- apply(ratio_exp_eachCellType,2, function(x) weighted.mean(x, pathway_number_weight/sum(pathway_number_weight)))
    mean_exp_pathway[cell_types]
  }
  shuffle_results <- matrix(unlist(tmp),ncol=length(cell_types),byrow = T) 
  rownames(shuffle_results) <- 1:5000
  colnames(shuffle_results) <- cell_types
  for(c in cell_types){
    if(is.na(mean_expression_shuffle[p,c])) next
    if(mean_expression_shuffle[p,c]>1){
      pval <- (1+ sum(shuffle_results[,c] > mean_expression_shuffle[p,c])) / 5000 
...
stopCluster(cl)
not_na <- apply(mean_expression_shuffle,1,function(x) sum(is.na(x)))
mean_expression_shuffle <- mean_expression_shuffle[not_na>0,]
#first look at the data,
dat <- mean_expression_shuffle[rowMaxs(mean_expression_shuffle,na.rm = T)>0,]
