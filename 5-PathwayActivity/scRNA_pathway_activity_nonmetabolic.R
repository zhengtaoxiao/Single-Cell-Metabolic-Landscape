...
pathway_file <- "../Data/nonMetabolic_KEGG.gmt"
...
gene_pathway_number <- num_of_pathways(pathway_file,rownames(selected_impute_sce))
...
dat[is.na(dat)] <- 1
pdf(file.path(outDir, "KEGGpathway_activity_heatmap_non_metabolic.pdf"),onefile=T,width=6,height=9)
mybreaks <- c(
...
write.table(mean_expression_noshuffle,file=file.path(outDir,"KEGGpathway_activity_noshuffle_nonmetabolic.txt"),row.names=T,col.names=T,quote=F,sep="\t")
write.table(mean_expression_shuffle,file=file.path(outDir,"KEGGpathway_activity_shuffle_nonmetabolic.txt"),row.names=T,col.names=T,quote=F,sep="\t")
write.table(pvalues_mat,file=file.path(outDir,"KEGGpathway_activity_shuffle_pvalue_nonmetabolic.txt"),row.names=T,col.names=T,quote=F,sep="\t")
...
ggsave(file.path(outDir,"pathway_activity_violinplot_nonmetabolic.pdf"),g,width = 2.5,height=1.5,units="in",device="pdf",useDingbats=FALSE)
